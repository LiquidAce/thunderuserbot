...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit("Connecting to official Thundergang server and analysing that img ...")
        try:
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "@thunderuserbot.png"
        ...
        await event.edit("Removed dat annoying Backgroup in {} seconds, powered by @thunderuserbot".format(ms))
    else:
        await event.edit("ReMove.BG API returned Errors. Please report to @thunderuserbot\n`{}".format(output_file_name.content.decode("UTF-8")))
